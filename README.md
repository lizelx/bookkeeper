...
## Информация по запуску для преподавателей: 

Проект выполнен с использованием poetry. В файле pyproject установлено 33 зависимости.
При тестировании приложения на другом компьютере (выполнение poetry install) возникли 
проблемы с PySide6 из-за path на винде. Команда poetry run bookkeeper\main.py заканчивалась быстрым 
открытием и закрытием главного окна приложения. Надеюсь, что при проверке задания таких технических
трудностей не возникнет.
main.py - главный файл приложения. В нем есть 2 режима открытия приложения: 
1) закомментированная 15 строчка main.py - запуск уже заполненной БД
2) закомментированная 14 строчка main.py - запуск пустой БД.
